import React from "react";
import "../css/departments.css";

const ITdepartment = () => {
  return (
    <div className="it-department">
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR WAQAR UL QOUNAIN</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/23-SWJ-148x150.png"
            alt="Dr Waqar ul Qounain"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">
              Chairman Department of Information Technology{" "}
            </li>
            <li className="item">Professor</li>
            <li className="item">
              PhD Computer Science (Vrije University, Amsterdam, The
              Netherlands)
            </li>
            <li className="item">
              M.Sc Computer Science (University of The Punjab, Pakistan)
            </li>
            <li className="item">
              PGD Computer Science (University of The Punjab, Pakistan)
            </li>
            <li className="item">
              Research / Teaching Interests: Computational Modelling, Artificial
              Intelligence, Data Mining and Machine Learning, Multi-Agent
              Systems
            </li>
            <li className="item">
              Email: swjaffry@pucit.edu.pk, chairman.dit@pucit.edu.pk,
              chairman.dit@pu.edu.pk
            </li>
            <li className="item">
              Website: http://faculty.pucit.edu.pk/swjaffry/{" "}
            </li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. MUHAMMAD KAMRAN MALIK</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/16-kamran-148x150.png"
            alt="Dr. Muhammad Kamran Malik"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Associate Professor</li>
            <li className="item">
              Program Coordinator, BS Information Technology (New Campus)
            </li>
            <li className="item">PhD Computer Science, PU</li>
            <li className="item">
              MS Software Project Management , NUCES-FAST, 2008
            </li>
            <li className="item">M.Sc Computer Science, PU, 2003</li>
            <li className="item">Email: kamran.malik@pucit.edu.pk</li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">ASIF SOHAIL</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/9-asif-sohail-148x150.png"
            alt="Asif Sohail"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">
              Program Coordinator, BS Information Technology (Old Campus)
            </li>
            <li className="item">
              PhD Computer Science (in progress) , PUCIT, University of the
              Punjab, Lahore
            </li>
            <li className="item">
              M.Phil Computer Science, PUCIT, University of the Punjab, Lahore
            </li>
            <li className="item">
              M.Sc. Computer Science, International Islamic University,
              Islamabad
            </li>
            <li className="item">
              Research / Teaching Interests: Database Systems, Operating
              Systems, Data Structures, Digital Logic & Design, Data Cleaning,
              Entity Resolution
            </li>
            <li className="item">Email: asif@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">MUHAMMAD ADEEL NISAR</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/adeel-148x150.png"
            alt="Muhammad Adeel Nisar"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">PhD Computer Science (in progress) , PU</li>
            <li className="item">M.Phil Computer Science, PU</li>
            <li className="item">M.Sc Computer Science, PU, 2004</li>
            <li className="item">
              Research / Teaching Interests: Operating Systems, Computer
              Architecture, System Programming
            </li>
            <li className="item">Email: Adeel.nisar@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">MUHAMMAD AHMAD GHAZALI</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/19-ahmadghazali-148x150.png"
            alt="Muhammad Ahmad Ghazali"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">PhD Computer Science (In Progress), PU</li>
            <li className="item">MS Computer Science, FAST-NU, 2004</li>
            <li className="item">BS Computer Science, FAST-NU, 2002</li>
            <li className="item">
              Research / Teaching Interests: Programming Fundamentals,
              Object-Oriented Programming, Data Structures & Algorithms, Theory
              of Automata, Discrete Mathematics
            </li>
            <li className="item">Email: ahmad.ghazali@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">SADEEQA RIAZ KHAN</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/34-sadeeqa-148x150.png"
            alt="Sadeeqa Riaz Khan"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">PhD Management Sciences (In Progress)</li>
            <li className="item">
              MS Total Quality Management, University of the Punjab, Lahore
            </li>
            <li className="item">
              MBIT (Marketing), University of the Punjab, Lahore
            </li>
            <li className="item">
              B.S Computer Science, Lahore College for Women University, Lahore
            </li>
            <li className="item">
              Research / Teaching Interests: Area of Interests: Management
              Information Systems, Organizational Behavior, Organizational
              Psychology, Professional Ethics
            </li>
            <li className="item">Email: sadeeqa@pucit.edu.pk</li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">KASHIF MURTAZA</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/37-kashiif-148x150.png"
            alt="Kashif Murtaza"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">PhD in Computer Vision (in progress), LUMS</li>
            <li className="item">
              MS in Computer Science, LUMS, Pakistan, 2012
            </li>
            <li className="item">
              BS in Computer Science, PUCIT, Pakistan, 2007
            </li>
            <li className="item">
              Research / Teaching Interests: Computer Vision, Pattern
              Recognition and Machine Learning, Multiple View Geometry, Convex
              Optimization
            </li>
            <li className="item">Email: kashif.murtaza@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">HAFIZ ANZAR AHMAD</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/43-anzar-148x150.png"
            alt="Hafiz Anzar Ahmad"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">
              Research / Teaching Interests: Agent Modeling, Intelligent
              Systems, AI, OOP, OOAD, Software Engineering, SW Architecture
              Pattern, Fundamentals of Programming Languages
            </li>
            <li className="item">Email: anzar@pucit.edu.pk</li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR.FARSIA HUSSAIN</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/41farsia-148x150.png"
            alt="Dr. Farsia Hussain"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">
              PhD Computational Mathematics (Spline Theory and Computer
              Graphics), PU, 2015
            </li>
            <li className="item">MS Mathematics, PU, 2008</li>
            <li className="item">BS Mathematics, PU, 2006</li>
            <li className="item">
              Research / Teaching Interests: Computational Mathematics, Linear
              Algebra, Calculus, Probability and Statistics
            </li>
            <li className="item">Email: Farsia.hussain@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. MIAN MUHAMMAD MUBASHER</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/42-mubshar-148x150.png"
            alt="Dr. Mian Muhammad Mubasher"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">
              PhD Computer Science, Department of Computer Science, University
              of the Punjab, Lahore
            </li>
            <li className="item">
              Research / Teaching Interests: Ambient Intelligence, Computational
              Modeling, Data and Text Mining, Information Retrieval, Social
              Simulation, Enterprise Application Development, Mobile Computing,
              Computational Modeling, Artificial Intelligence
            </li>
            <li className="item">Email: mubasher@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. MUHAMMAD FAROOQ</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2021/06/farooqs-148x150.png"
            alt="Dr. Muhammad Farooq"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor (Adhoc)</li>
            <li className="item">
              Ph.D., Computer Science, Asian Institute of Technology (AIT),
              Thailand
            </li>
            <li className="item">
              M.Phil, Computer Science, University of the Punjab (PU), Lahore
            </li>
            <li className="item">
              M.Sc., Computer Science, University of Engineering and Technology
              (UET), Lahore
            </li>
            <li className="item">
              B.Sc., Agricultural Engineering, University of Agriculture (UAF),
              Faisalabad
            </li>
            <li className="item">
              Research / Teaching Interests: Computer Vision, Image Processing,
              Machine Learning, Network Programming
            </li>
            <li className="item">Email: Mfarooq@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">FAKHRA JABEEN</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/48-fakhra-148x150.png"
            alt="Fakhra Jabeen"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Lecturer</li>
            <li className="item">
              PhD – Computer Science (In progress) Vrije University, Amsterdam,
              The Netherlands.
            </li>
            <li className="item">
              MS – Computer Science. University of Central Punjab, Lahore
            </li>
            <li className="item">
              M.Sc Computer Science, International Islamic University,
              Islamabad, 2004
            </li>
            <li className="item">
              Research / Teaching Interests: Programming in C/C++/VC++, Computer
              Organization
            </li>
            <li className="item">Email: fakhra@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">MARYAM NAWAZ AWAN</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/54-maryam_nawaz-148x150.png"
            alt="Maryam Nawaz Awan"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Lecturer</li>
            <li className="item">PhD Physics (PU) thesis in progress</li>
            <li className="item">MPhil Solid State Physics (PU) 2011</li>
            <li className="item">Bsc(hons) Physics (PU) 2009</li>
            <li className="item">
              Research / Teaching Interests: Theoretical Atomic and Molecular
              Physics
            </li>
            <li className="item">Email: maryam.nawaz@pucit.edu.pk</li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. ZARA NASAR</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/10/zara-148x150.png"
            alt="Dr. Zara Nasar"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Lecturer</li>
            <li className="item">
              PhD Computer Science, PUCIT, University of The Punjab, Pakistan
            </li>
            <li className="item">
              M.Phil. Computer Science, 2014-2016 (PUCIT, University of The
              Punjab, Pakistan)
            </li>
            <li className="item">
              BS Software Engineering, 2009-2013 (PUCIT, University of The
              Punjab, Pakistan)
            </li>
            <li className="item">
              Research / Teaching Interests: Data Mining and Machine Learning,
              Natural Language Understanding and Processing, Artificial
              Intelligence, Computational Modelling
            </li>
            <li className="item">Email: zara.nasar@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
    </div>
  );
};

export default ITdepartment;
