import React from "react";
import "../css/departments.css";

const DSdepartment = () => {
  return (
    <div className="ds-department">
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. SHAHID MANZOOR</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2021/08/shahid-manzoor-148x150.png"
            alt="Dr. Shahid Manzoor"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Chairman Department of Data Science</li>
            <li className="item">Associate Professor</li>
            <li className="item">
              PhD in Bioinformatics, Swedish University of Agricultural Science,
              Uppsala, Sweden
            </li>
            <li className="item">
              MS in Bioinformatics, Swedish University of Agricultural Science,
              Uppsala, Sweden
            </li>
            <li className="item">
              M.Sc in Computer Science, University of Agriculture, Faisalabad,
              Pakistan
            </li>
            <li className="item">
              B.Sc (Hons), University of Agriculture, Faisalabad, Pakistan
            </li>
            <li className="item">
              Research / Teaching Interests: Bioinformatics, Comparative
              Genomics, Genome Assembly and Annotation, Metagenomics, Biological
              Databases, Correlation network analysis, Microbiome analysis,
              Proteomic, Transcriptomic and Metabolic Pathway analysis, Database
              Systems, Database Administration
            </li>
            <li className="item">
              Email: chairman.dds@pucit.edu.pk, chairman.dds@pu.edu.pk
            </li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. MUHAMMAD NADEEM MAJEED</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2021/03/nadeemjaed-148x150.png"
            alt="Dr. Muhammad Nadeem Majeed"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Associate Professor </li>
            <li className="item">
              Ph.D. in Computer Engineering, University of Engineering and
              Technology, Taxila.
            </li>
            <li className="item">
              MS in Computer Engineering, CASE, University of Engineering and
              Technology, Taxila.
            </li>
            <li className="item">
              Research / Teaching Interests: Communication & Networks, Data
              Science and Machine learning, Business Intelligence &
              Strategy-Making.
            </li>
            <li className="item">Email: nadeem.majeed@pucit.edu.pk</li>
            <li className="item">
              Google Meet: https://meet.google.com/jjt-vaxr-rub
            </li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. SYED FAISAL BUKHARI</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2021/05/syed-faisal-bukhari-148x150.png"
            alt="Dr. Syed Faisal Bukhari"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Associate Professor</li>
            <li className="item">
              Ph.D., Computer Science, Asian Institute of Technology (AIT),
              Thailand
            </li>
            <li className="item">
              M.S., Computer Science, Asian Institute of Technology (AIT),
              Thailand
            </li>
            <li className="item">M.Sc., Computer Science, Punjab University</li>
            <li className="item">M.Sc., Statistics, Punjab University</li>
            <li className="item">
              Research / Teaching Interests: Computer Vision, Image Processing,
              Machine Learning, and Statistics
            </li>
            <li className="item">Email: faisal.bukhari@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. KHURRAM SHAHZAD</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/27-khuram-148x150.png"
            alt="Dr Khurram Shahzad"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Associate Professor</li>
            <li className="item">
              PhD Computer and Systems Sciences, KTH – Royal Institute of
              Technology, Sweden
            </li>
            <li className="item">
              MS Information Technology, KTH – Royal Institute of Technology,
              Sweden, 2006
            </li>
            <li className="item">M.Sc Computer Science, PU, 2004</li>
            <li className="item">
              Research / Teaching Interests: Data Warehousing, Business Process
              Management and Academic Social Network
            </li>
            <li className="item">Email: khurram@pucit.edu.pk</li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. WAHEED IQBAL</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/28-waheed-148x150.png"
            alt="Dr. Waheed Iqbal"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Associate Professor</li>
            <li className="item">
              PhD Computer Science Asian Institute of Technology, Thailand
            </li>
            <li className="item">
              Research / Teaching Interests: Cloud Computing, Distributed
              Systems, Machine Learning, Data Mining, Mobile and Web Computing,
              Scalable Applications and Architectures, Algorithms Analysis, Data
              Structures, Operating Systems
            </li>
            <li className="item">Website: http://www.waheediqbal.info/</li>
            <li className="item">Email: waheed.iqbal@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">
            MAJ. (RETD.) DR. MUHAMMAD ARIF BUTT
          </span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/arif-butt-148x150.png"
            alt="Maj. (Retd.) Dr.
Muhammad Arif Butt"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor </li>
            <li className="item">
              PhD Computer Science PUCIT, University of the Punjab, Lahore
            </li>
            <li className="item">M.Phil Computer Science, PU, (Gold Medal)</li>
            <li className="item">
              M.Sc Computer Science, PU, 2005 (Gold Medal)
            </li>
            <li className="item">JSC (mil), SI&T, Quetta</li>
            <li className="item">B.Sc PMA, Kakul</li>
            <li className="item">
              Research / Teaching Interests: OS Kernel and device driver
              development, Fuzzification of OS Kernel Modules, Embedded Systems
              and OS Security
            </li>
            <li className="item">
              Video Lectures SP with Linux, OS with Linux, Internetworking with
              Linux, C-Refresher Computer Organization and Assembly Language
              Programming{" "}
            </li>
            <li className="item">Email: arif@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. MUHAMMAD IDREES</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/idrees-148x150.png"
            alt="Dr. Muhammad Idrees"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">
              PhD Computer Science PUCIT, University of the Punjab, Lahore
            </li>
            <li className="item">
              M.Phil Computer Science, PU, 2010 (First Position)
            </li>
            <li className="item">
              M.Sc Computer Science, PU, 1995 (3rd Position)
            </li>
            <li className="item">M.Sc Physics, PU, 1992</li>
            <li className="item">
              Research / Teaching Interests: Programming (C++/JAVA/.NET),
              Database Design and Administration, Data Structures, Design and
              Analysis of Algorithms, Theory of Automata.
            </li>
            <li className="item">Email: idrees@pucit.edu.pk</li>
            <li className="item">
              Website: https://sites.google.com/site/siridrees/
            </li>
            <li className="item">
              YouTube: https://www.youtube.com/c/siridrees
            </li>
            <li className="item">
              Facebook: https://www.facebook.com/sir.idrees/
            </li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">TARIQ MAHMOOD BUTT</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/tariq-but-148x150.png"
            alt="Tariq Mahmood Butt"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">
              M.Sc Electrical Engineering UET Lahore (Thesis in progress)
            </li>
            <li className="item">
              M.Sc Computer Science, Al-Khair University , Lahore, 2002
            </li>
            <li className="item">MBA, Punjab University, 1998</li>
            <li className="item">
              B.Sc Electrical Engineering, UET Lahore, 1992
            </li>
            <li className="item">
              Research / Teaching Interests: Digital Logic and Design,
              Microprocessors, Computer Architecture, Operating Systems
            </li>
            <li className="item">Email: tariq.butt@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. SYED MUHAMMAD ALI</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/smali-148x150.png"
            alt="Dr. Syed Muhammad Ali"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">
              PhD Computer Science, University of Central Florida, USA, 2016
            </li>
            <li className="item">
              MS Computer Science, University of Central Florida, USA, 2014
            </li>
            <li className="item">
              M.Sc Computer Science, University of the Punjab, Lahore, 2003
            </li>
            <li className="item">
              B.Sc Engineering, University of Engg. & Tech. (UET), Lahore, 2000
            </li>
            <li className="item">
              Research / Teaching Interests: Machine Learning, Image Processing,
              Computer Vision
            </li>
            <li className="item">Email: muhammad.ali@pucit.edu.pk</li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. ATHER ASHRAF</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/14-ather-148x150.png"
            alt="Dr. Ather Ashraf"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">PhD. Computer Science, PU</li>
            <li className="item">
              M.S. Computer Science, UCP, Lahore, 2003 (First Position)
            </li>
            <li className="item">B.Sc. Civil Engineering, UET, Lahore, 2000</li>
            <li className="item">M.Sc. GIS, Punjab University, 2003</li>
            <li className="item">
              Research / Teaching Interests: Geo-Spatial System Design,
              Geo-Spatial data bases & Web GIS
            </li>
            <li className="item">Email: ather@pucit.edu.pk</li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">IMRAN JAVED</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/17-imranjaved-148x150.png"
            alt="Imran Javed"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">PhD Computer Science (In Progress)</li>
            <li className="item">M.Phil Computer Science, PU</li>
            <li className="item">M.Sc Computer Science, PU, 2001</li>
            <li className="item">PGD in Computer Science, PU, 1999</li>
            <li className="item">
              Research / Teaching Interests: Theory of Formal Languages and
              Automata, Design and Implementation of Compilers, Programming
              Languages
            </li>
            <li className="item">Email: Imranj@pucit.edu.pk</li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. ZUBAIR NAWAZ</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/22-znawaz-148x150.png"
            alt="Dr Zubair Nawaz"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor</li>
            <li className="item">
              PhD Computer Engineering, Delft University of Technology, The
              Netherlands, 2011
            </li>
            <li className="item">
              MS Computer Science, Lahore University of Management Sciences,
              Pakistan, 2002
            </li>
            <li className="item">
              BS Mechanical Engineering, Ghulam Ishaq Khan Institute of
              Engineering and Technology, Pakistan, 1997
            </li>
            <li className="item">
              Research / Teaching Interests: Analysis of Algorithms, Advanced
              Algorithms, High Performance Computing
            </li>
            <li className="item">Email: znawaz@pucit.edu.pk</li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. RUKHSANA KAUSAR</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/04/45-rukhsana-148x150.png"
            alt="Dr.Rukhsana Kausar"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor (Adhoc)</li>
            <li className="item">
              PhD in Applied Mathematics , Systems and controls theory Group,
              Department of Mathematics, University of Kaiserslautern, Germany.
            </li>
            <li className="item">
              MS in Industrial Mathematics(scientific computing) , Bio
              Mathematics Group, Department of Mathematics, University of
              Kaiserslautern, Germany.
            </li>
            <li className="item">
              PGD (Computer Science), Govt, College University, Lahore, Pakistan
            </li>
            <li className="item">
              M.Sc (Applied Mathematics), Department of Mathematics, University
              of the Punjab, Lahore, Pakistan.
            </li>
            <li className="item">
              Research / Teaching Interests: Modeling and analysis of networks,
              Switched systems, Hybrid systems, Artificial Intelligence, Neural
              networks, Differential algebraic equations, Numerics of
              hyperbolics systems for gas networks,Higher order numerics for
              modeling via Partial differential equations, Modeling via
              stochastic differential equations,Probability and algorithms,
              Machine learning, Atomisation and controls, Finite element
              analysis software
            </li>
            <li className="item">Email: rukhsana@pucit.edu.pk</li>
            <li className="item">Curriculum Vitae</li>
          </ul>
        </div>
      </div>
      <div className="person">
        <div className="person-name-img">
          <span className="person-name">DR. MUHAMMAD ABDULLAH</span>
          <img
            src="https://pucit.edu.pk/wp-content/uploads/2019/10/abdulahs-148x150.png"
            alt="Dr. Muhammad Abdullah"
          />
        </div>
        <div className="person-info">
          <ul>
            <li className="item">Assistant Professor (Adhoc)</li>
            <li className="item">
              Ph.D. Computer Science PUCIT, University of the Punjab (2020)
            </li>
            <li className="item">
              M.Phil Computer Science, PUCIT, University of the Punjab (2016)
            </li>
            <li className="item">
              BS Computer Science, University of the Punjab (2014)
            </li>
            <li className="item">
              Teaching: Programming Fundamentals, Object-Oriented Programming,
              Analysis of Algorithms, Computer Architecture, Cloud Computing
            </li>
            <li className="item">
              Research / Teaching Interests: Cloud Computing, Scalable
              Applications, Machine Learning, System Performance Management.
            </li>
            <li className="item">Email: muhammad.abdullah@pucit.edu.pk</li>
          </ul>
        </div>
      </div>
    </div>
  );
};

export default DSdepartment;
